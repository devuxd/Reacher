...
public class ReacherNodeRenderer extends LabelRenderer {
	public static String RENDER_TYPE = "render_type";
	public static enum NodeType { METHODS, STMTS };
	
	private ShapeRenderer shapeRenderer = new ShapeRenderer();	   
    private double borderSize = 4;			// Border size between text edge and filled region edge
	private NodeType nodeType;
    
	public ReacherNodeRenderer(NodeType nodeType) 
	{
		super();
		this.nodeType = nodeType;		
	}

	public ReacherNodeRenderer(String textField, NodeType nodeType) 
	{
		super(textField);
		this.nodeType = nodeType;
	}

	public ReacherNodeRenderer(String textField, String imageField, NodeType nodeType) 
	{
    ...
    protected Shape getRawShape(VisualItem item) {
        ...
        // Try to use the preferred width, if there is one. Otherwise, just use the bounds width.
        double preferredWidth = item.getDouble(ReacherDisplay.PREFERRED_WIDTH); 
        double width = preferredWidth == 0 ? bounds.getWidth() : preferredWidth;
    ...
    public void render(Graphics2D g, VisualItem item) {
        ...
        {
        	// Get the colors for the item
        	Node node = ReacherDisplay.Instance.getNodeForItem(item);
        	ArrayList<Search> highlightingSearches;
        	if (nodeType == NodeType.METHODS)
        		highlightingSearches = ReacherDisplay.Instance.getMethodParams(node).methodNode.getHighlightingSearches();
        	else
        		highlightingSearches = ReacherDisplay.Instance.getStmtParams(node).stmtNode.getHighlightingSearches();

        	
        	if (item.isHighlighted())
        	{
				item.setFillColor(ColorLib.color(Color.ORANGE));
				item.setTextColor(ColorLib.color(Color.BLACK));  
				
                GraphicsLib.paint(g, item, shape, getStroke(item), RENDER_TYPE_FILL);
        	}
        	else if (!highlightingSearches.isEmpty())
        	{
        		int i = 0;
        		for (RectangularShape region : GraphicsUtils.splitVertically(shape, highlightingSearches.size()))
        		{
        			item.setFillColor(ColorLib.color(highlightingSearches.get(i).getColor()));        			
                    GraphicsLib.paint(g, item, region, getStroke(item), RENDER_TYPE_FILL);        			
        			i++;
        		}
        	...
        	{
        		// If there are no highlighting searches, reset the color to the default.
