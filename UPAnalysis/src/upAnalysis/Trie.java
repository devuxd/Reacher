 ...
 * 
 * Trie implementation in Prefuse modified by Thomas LaToza to make it generic.
 * 
 */
public class Trie<V> {
     ...
     */
    public class TrieNode implements Iterable<V> {
        boolean isLeaf;
        int leafCount = 0;
    ...
    public class TrieBranch extends TrieNode {
        ...
        TrieNode[] children = new Trie.TrieNode[1];
    }
    ...
    public class TrieLeaf extends TrieNode {
        public TrieLeaf(String word, V t) {
            this.word = word;
            tuple = t;
            next = null;
            leafCount = 1;
     ...
     */
    public class TrieIterator implements Iterator<V> {
    ...
    /**
     * Add a new word to the trie, associated with the given value.
     * @param word the word to add to the Trie
     * @param t the value associated with the word
     */
    public void addString(String word, V t) {
        TrieLeaf leaf = new TrieLeaf(word,t);
        addLeaf(root, leaf, 0);
    }
    
    /**
     * Remove a word/value pair from the trie.
     * @param word the word to remove
     * @param t the associated value to remove
     */
    public void removeString(String word, V t) {
    ...
    private boolean removeLeaf(TrieBranch b, String word, V t, int depth) {
        ...
        if ( i == -1 ) {
            ...
            if ( n instanceof Trie.TrieBranch ) {
    ...
    private void repairBranch(TrieBranch b, int i) {
        if ( i == 0 ) {
            ...
            TrieNode[] nkids = new Trie.TrieNode[len-1];
    ...
    private void addLeaf(TrieBranch b, TrieLeaf l, int depth) {
        ...
        if ( i == -1 ) {
            ...
            if ( n == null ) {
                ...
                b.children[i] = l;
            } else if ( n instanceof Trie.TrieBranch ) {
    ...
    private TrieNode find(String word, TrieBranch b, int depth) {
        ...
        } else if ( b.children[i] instanceof Trie.TrieLeaf ) {
