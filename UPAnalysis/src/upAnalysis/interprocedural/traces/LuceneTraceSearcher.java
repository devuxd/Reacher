...
public class TraceSearcher 
{
	...
	private void indexSummary(MethodSummary summary)
	{
	...
	{		
		...
		addStringToIndex(stmt.getTargetMember().getElementName(), stmt, null);		
		
		// 2. Add the target declaring type name
		if (targetDeclaringType != null)
			addStringToIndex(targetDeclaringType.getElementName(), stmt, null);
		
		// 3. Add the package qualifier of the declaring type
		if (targetDeclaringType != null)
			addStringToIndex(targetDeclaringType.getPackageFragment().getElementName(), stmt, null);
	}
	...
	{		
		...
		{				
			...
			doc.add(new Field(FULL_TEXT, text.substring(i).trim(), Field.Store.NO, Field.Index.TOKENIZED));
